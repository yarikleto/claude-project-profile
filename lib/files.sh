...
_canonical_existing_path() {
  ...
  while [[ $depth -lt 40 ]]; do
    ...
    if [[ -L "$path" ]]; then
      local link_dest
      link_dest="$(readlink "$path" 2>/dev/null)" || return 1
      if [[ "$link_dest" == /* ]]; then
        path="$link_dest"
      else
        path="$real_dir/$link_dest"
      fi
      depth=$((depth + 1))
      continue
    fi
    ...
    real_dir="$(cd "$(dirname "$path")" 2>/dev/null && pwd -P)" || return 1
    echo "$real_dir/$(basename "$path")"
    return
  done

  err "SAFETY: too many symlink levels while resolving '$1'"
  return 1
}
...
_assert_path_within() {
  ...
  if [[ -e "$target" || -L "$target" ]]; then
    local real_target
    real_target="$(readlink -f "$target" 2>/dev/null || realpath "$target" 2>/dev/null || _canonical_existing_path "$target")" || return 1
